...
<section class="section">
  <div class="row">
    <div class="col-lg-12">
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">Vessel List</h5>
                ...
                <?php
                  ...
                  if ($err) {     
                    ...
                    foreach ($shippingCompanies as $i => $shippingCompany) {
                      ...
                      echo "<tr>";
                        echo "<td>{$i}</td>";
                        $logo = !empty($shippingCompany['image']) ? $shippingCompany['image'] : 'mhei_logo/default.jpg';
                        echo "<td>
                          <a href='{$logo}' target='_blank'>
                              <img src='{$logo}' alt='School Logo' style='width:50px; height:auto;'>
                          </a>
                        </td>";
                        $imo_registry_number = $shippingCompany['imo_number'].($shippingCompany['imo_number'] != '' && $shippingCompany['registry_number'] != ''?'/':'').$shippingCompany['registry_number'];
                        echo"<td>{$imo_registry_number}</td>
                        <td>{$shippingCompany['vessel_name']}</td>
                        <td>{$shippingCompany['port_of_registry']}</td>
                        <td>{$shippingCompany['vessel_type']}</td>
                        <td>{$shippingCompany['grt']}</td>
                        <td>{$shippingCompany['kw']}</td>
                        <td>{$shippingCompany['flag']}</td>
                        <td>{$shippingCompany['route']}</td>";
